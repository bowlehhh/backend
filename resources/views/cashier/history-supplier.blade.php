...
<head>
    ...
    <title>History Supplier - Toko Pak Paul</title>
...
<div class="h-screen overflow-hidden bg-[#f7f9fb]">
    ...
    <main class="lg:ml-[260px] h-full overflow-y-auto p-4 lg:p-6">
        <div class="mb-4 flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-extrabold">Kelompok Transaksi per PT</h2>
        ...
        @forelse($groups as $group)
            <div class="mb-5 overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <div class="border-b border-slate-200 px-4 py-3">
                    <h3 class="text-xl font-extrabold text-slate-900">{{ $group['supplier_name'] }}</h3>
                    <p class="text-sm text-slate-500">Riwayat transaksi kasir dikelompokkan per PT/supplier.</p>
                </div>
                <div class="overflow-x-auto">
                    <table class="min-w-full text-sm">
                        <thead class="bg-slate-50">
                            <tr>
                                <th class="px-4 py-3 text-left font-semibold text-slate-600">Total Transaksi</th>
                                <th class="px-4 py-3 text-left font-semibold text-slate-600">Total Qty</th>
                                <th class="px-4 py-3 text-left font-semibold text-slate-600">Total Nilai</th>
                                <th class="px-4 py-3 text-left font-semibold text-slate-600">Kredit</th>
                                <th class="px-4 py-3 text-left font-semibold text-slate-600">Lunas</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="border-t border-slate-100">
                                <td class="px-4 py-3 font-bold">{{ number_format((int) $group['summary']['total_transaksi']) }} kali</td>
                                <td class="px-4 py-3 font-bold">{{ number_format((int) $group['summary']['total_qty']) }}</td>
                                <td class="px-4 py-3 font-bold">{{ $group['summary']['total_nilai'] }}</td>
                                <td class="px-4 py-3">{{ number_format((int) $group['summary']['kredit']) }}</td>
                                <td class="px-4 py-3">{{ number_format((int) $group['summary']['lunas']) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        @empty
            <div class="rounded-2xl border border-slate-200 bg-white px-4 py-8 text-center text-slate-500">
                Belum ada transaksi kasir untuk dikelompokkan per supplier.
            </div>
        @endforelse
    </main>
</div>
