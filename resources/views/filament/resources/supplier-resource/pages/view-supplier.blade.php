...
<x-filament-panels::page>
          ...
          <main class="sf-content min-h-screen p-4 md:p-6">
            ...
            <div class="sf-toolbar bg-surface-container-lowest p-3 md:p-4 rounded-t-xl border-x border-t border-outline-variant flex flex-wrap items-center gap-3 md:gap-4">
              <div class="flex flex-col gap-1 min-w-0 flex-1">
                ...
                <input id="globalSearchInput" type="text" value="{{ $viewData['search'] ?? '' }}" placeholder="Cari part number, nama barang, kategori, brand, barcode..." class="bg-surface border border-outline-variant rounded-lg px-4 py-2 text-label-md focus:ring-primary focus:border-primary" oninput="clearTimeout(deleteTimeout); deleteTimeout = setTimeout(applyFilters, 500)">
              </div>
                      ...
                      <tr class="hover:bg-surface-container-low transition-colors group" data-product-id="{{ (int) ($product['id'] ?? 0) }}">
                        <td class="px-6 py-4">
                          <div class="flex items-center gap-3">
                            ...
                            <div>
                              <p class="font-bold text-on-surface text-base">{{ $product['sku'] ?? '-' }}</p>
                              <p class="text-xs text-on-surface-variant mt-0.5">{{ $product['name'] ?? '-' }}</p>
                            </div>
                          </div>
                        </td>
    ...
    <script>
      ...
      function focusBrandFilter() {
        const target = document.getElementById('globalSearchInput');
        if (!target) return;
        target.scrollIntoView({ behavior: 'smooth', block: 'center' });
        target.focus();
      }

      function focusCategoryFilter() {
        const target = document.getElementById('globalSearchInput');
        if (!target) return;
        target.scrollIntoView({ behavior: 'smooth', block: 'center' });
        target.focus();
        ...
        lowStockList.innerHTML = lowStockProducts.map((item) => `
          <div class="rounded-xl border border-outline-variant bg-surface px-4 py-4">
            <div class="flex items-start justify-between gap-3">
              <div>
                <p class="font-semibold text-on-surface">${escapeHtml(item.sku || item.product_sku || '-')}</p>
      ...
      function exportToCSV() {
        ...
        table.querySelectorAll('thead th').forEach((th) => {
          ...
          if (text !== 'Aksi') headers.push(text);
        });
        ...
        table.querySelectorAll('tbody tr').forEach((tr) => {
          ...
          tds.forEach((td, index) => {
            ...
            const mainValue = td.querySelector('p.font-bold');
            if (mainValue) text = mainValue.textContent.trim();
      ...
      function openEditModal(product) {
        resetFormState(editForm, 'editFormAlert');
        fillProductForm(editForm, product);
      ...
      async function submitProductForm(form, url, method, alertId) {
        ...
        if (response.status === 422) {
          showFormErrors(form, alertId, payload.errors || {});
          return null;
        }
        if (!response.ok) {
          showFormAlert(alertId, payload.message || 'Terjadi kesalahan saat menyimpan data.');
          return null;
        }
        return payload;
      }
      ...
      function showToast(message, type = 'success') {
        ...
        toast.classList.remove('hidden', 'border-primary', 'border-error', 'text-primary', 'text-error');
          ...
          <tr class="hover:bg-surface-container-low transition-colors group" data-product-id="${product.id}">
            <td class="px-6 py-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-bold text-on-surface text-base">${escapeHtml(product.sku || '-')}</p>
                  <p class="text-xs text-on-surface-variant mt-0.5">${escapeHtml(product.name || '-')}</p>
                </div>
              </div>
            </td>
      ...
      function syncNotificationProducts(product) {
        ...
        if (index >= 0) {
          productsForNotification[index] = product;
          return;
        }
      ...
      function refreshCountText() {
        const visibleRows = tableBody.querySelectorAll('tr[data-product-id]').length;
        countText.textContent = `Menampilkan ${visibleRows > 0 ? 1 : 0}-${visibleRows} dari ${pageMeta.total} barang`;
      }
      ...
      editForm.addEventListener('submit', async function (event) {
        event.preventDefault();
        const productId = editForm.querySelector('[name="product_id"]').value;
        const payload = await submitProductForm(editForm, `${updateUrlBase}/${productId}`, 'PUT', 'editFormAlert');
        if (!payload) return;
        upsertProductRow(payload.product, 'update');
        closeEditModal();
        showToast(payload.message || 'Barang berhasil diperbarui.');
      });
