<div id="admin-logout-modal" class="fixed inset-0 z-[80] hidden items-center justify-center px-4">
    ...
    <div class="relative w-full max-w-md overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl">
        <div class="flex items-start gap-4 border-b border-slate-200 bg-gradient-to-r from-rose-50 to-amber-50 px-5 py-4">
            ...
            <div class="min-w-0">
                ...
                <p class="mt-1 text-sm text-slate-600">Sesi admin akan berakhir dan Anda perlu login lagi untuk melanjutkan.</p>
            </div>
        </div>

        <div class="px-5 py-4">
            <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                Pastikan semua perubahan yang sedang dibuka sudah disimpan sebelum logout.
            </div>
        </div>

        <div class="flex items-center justify-end gap-3 border-t border-slate-200 bg-slate-50 px-5 py-4">
            <button type="button" id="admin-logout-modal-cancel" class="rounded-xl border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">
                Batal
            </button>
            <button type="button" id="admin-logout-modal-confirm" class="inline-flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-700">
...
<script>
    (function () {
        ...
        const cancelBtn = document.getElementById('admin-logout-modal-cancel');
        const confirmBtn = document.getElementById('admin-logout-modal-confirm');
        ...
        document.querySelectorAll('.js-admin-logout-form').forEach((form) => {
...
<?php /**PATH /home/mrgana/pos-inventory/backend/resources/views/filament/partials/logout-modal.blade.php ENDPATH**/ ?>