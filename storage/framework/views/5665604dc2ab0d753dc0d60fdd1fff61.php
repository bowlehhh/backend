...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
        ...
        <div class="sf-layout">
          <aside class="sf-sidebar hidden lg:flex flex-col w-full p-4 bg-surface">
            ...
            <nav class="flex-1 space-y-1 overflow-y-auto pr-1">
              ...
              <a class="sf-nav-item w-full flex items-center gap-3 text-on-surface-variant px-3 py-2 hover:bg-surface-container-high transition-all rounded-lg font-medium text-left" href="<?php echo e(url('/admin/admin-module?type=taxonomy')); ?>"><span class="material-symbols-outlined">category</span><span>Kategori & Brand</span></a>
          ...
          <main class="sf-content min-h-screen p-4 md:p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $stats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $stat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                <div class="bg-surface-container-lowest p-6 rounded-xl border border-outline-variant custom-shadow <?php echo e($index === 1 ? 'cursor-pointer hover:shadow-md transition-shadow' : ''); ?>" <?php if($index===1): ?> onclick="openLowStockModal()" <?php endif; ?>>
                  <div class="flex items-center gap-3 mb-2">
                    <div class="p-1 rounded-lg <?php echo e($index === 0 ? 'bg-primary-container/20' : ($index === 1 ? 'bg-tertiary-container/20' : 'bg-secondary-container/20')); ?>">
                      <span class="material-symbols-outlined <?php echo e($index === 0 ? 'text-primary' : ($index === 1 ? 'text-tertiary' : 'text-secondary')); ?>"><?php echo e($stat['icon'] ?? 'inventory_2'); ?></span>
                    </div>
                    <span class="text-on-surface-variant font-medium"><?php echo e($stat['label'] ?? '-'); ?></span>
                  </div>
                  <div class="sf-value <?php echo e($index === 0 ? 'text-primary' : ($index === 1 ? 'text-tertiary' : 'text-secondary')); ?> font-bold leading-tight"><?php echo e($stat['value'] ?? '0'); ?></div>
                  <p class="text-sm mt-1 font-medium <?php echo e($index === 0 ? 'text-primary' : ($index === 1 ? 'text-tertiary' : 'text-secondary')); ?>"><?php echo e($stat['description'] ?? '-'); ?></p>
                </div>
                      ...
                      <tr class="hover:bg-surface-container-low transition-colors group" data-product-id="<?php echo e((int) ($product['id'] ?? 0)); ?>">
                        ...
                        <td class="px-6 py-4">
                          <span class="px-3 py-1 rounded-full <?php echo e($low ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'); ?> text-label-sm font-semibold inline-flex items-center gap-1">
                            <span class="w-1.5 h-1.5 rounded-full <?php echo e($low ? 'bg-red-500' : 'bg-emerald-500'); ?>"></span><?php echo e($stock); ?> Unit
                          </span>
                        </td>
    ...
    <form id="logoutForm" method="POST" action="<?php echo e(url('/admin/logout')); ?>" class="hidden">
      <?php echo csrf_field(); ?>
    </form>
    ...
    <script>
      ...
      const storeUrl = <?php echo json_encode(route('admin.dashboard.products.store'), 15, 512) ?>;
      const updateUrlBase = <?php echo json_encode(url('/admin/dashboard/products'), 15, 512) ?>;
      const csrfToken = <?php echo json_encode(csrf_token(), 15, 512) ?>;
      const productsForNotification = <?php echo json_encode($products, 15, 512) ?>;
