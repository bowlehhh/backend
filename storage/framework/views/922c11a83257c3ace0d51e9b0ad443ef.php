...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
    ...
    <style>
      ...
      html.sf-dashboard-page, .sf-dashboard-page, .sf-dashboard-page body, .sf-dashboard-page .fi-body, .sf-dashboard-page .fi-layout, .sf-dashboard-page .fi-main, .sf-dashboard-page .fi-main-ctn { background: #f7f9fb !important; height: 100% !important; overflow: hidden !important; }
      ...
      .sf-layout { display: grid; grid-template-columns: 240px minmax(0, 1fr); height: calc(100vh - 64px); }
      .sf-sidebar { position: sticky; top: 64px; height: calc(100vh - 64px); border-right: 1px solid #d4dbd7; overflow: hidden; }
      .sf-main-scroll { height: calc(100vh - 64px); overflow-y: auto; }
      .sf-nav-item { font-size: 14px; }
      @media (max-width: 1279px) {
        .sf-layout { grid-template-columns: 1fr; height: auto; }
    ...
    <div class="sf-wrap bg-[#f7f9fb] text-[#191c1e] h-screen w-screen max-w-none ml-[calc(50%-50vw)] mr-[calc(50%-50vw)] overflow-hidden">
      ...
      <div class="sf-layout">
        <aside class="sf-sidebar lg:flex flex-col w-full p-4 pb-6 bg-white hidden">
          ...
          <nav class="flex-1 min-h-0 space-y-1 overflow-y-auto">
            ...
            <a class="sf-nav-item w-full flex items-center gap-3 text-[#47534d] px-3 py-2 hover:bg-[#eceef0] transition-all rounded-lg font-medium text-left" href="<?php echo e(url('/admin/admin-module?type=batches')); ?>">
              <span class="material-symbols-outlined">layers</span><span>Batch Barang</span>
            </a>
            <a class="sf-nav-item w-full flex items-center gap-3 text-[#47534d] px-3 py-2 hover:bg-[#eceef0] transition-all rounded-lg font-medium text-left" href="<?php echo e(url('/admin/admin-module?type=taxonomy')); ?>">
        ...
        <main class="sf-main-scroll p-4 md:p-6">
          <div class="flex items-center justify-between mb-6">
            <div>
              <h1 class="text-4xl font-semibold">Daftar Supplier</h1>
              <p class="text-[#52615a]">Kelola data supplier dengan tampilan yang konsisten.</p>
            </div>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
            <div class="bg-[#ffffff] border border-[#b8c5be] rounded-2xl p-6 shadow-[0_12px_30px_rgba(15,23,42,0.08)]">
              <p class="text-base text-[#415149] font-medium">Total Supplier</p>
              <p class="text-5xl leading-tight font-extrabold text-[#006948] mt-2"><?php echo e(number_format($stats['total'] ?? 0, 0, ',', '.')); ?></p>
            </div>
            <div class="bg-[#ffffff] border border-[#b8c5be] rounded-2xl p-6 shadow-[0_12px_30px_rgba(15,23,42,0.08)]">
              <p class="text-base text-[#415149] font-medium">Supplier Aktif</p>
              <p class="text-5xl leading-tight font-extrabold text-[#825100] mt-2"><?php echo e(number_format($stats['active'] ?? 0, 0, ',', '.')); ?></p>
            </div>
            <div class="bg-[#ffffff] border border-[#b8c5be] rounded-2xl p-6 shadow-[0_12px_30px_rgba(15,23,42,0.08)]">
              <p class="text-base text-[#415149] font-medium">Total Stok Supplier</p>
              <p class="text-5xl leading-tight font-extrabold text-[#4648d4] mt-2"><?php echo e(number_format($stats['total_stock'] ?? 0, 0, ',', '.')); ?></p>
            </div>
          </div>
