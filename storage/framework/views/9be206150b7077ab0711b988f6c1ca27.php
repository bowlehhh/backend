...
<div class="h-screen overflow-hidden bg-[#f7f9fb]">
    <aside class="hidden lg:flex fixed inset-y-0 left-0 z-30 w-[340px] flex-col border-r border-slate-300 bg-white">
        ...
        <div class="min-h-0 flex-1 overflow-y-auto custom-scrollbar px-4 py-4 space-y-4">
            <nav class="space-y-2">
                <a href="<?php echo e(route('cashier.dashboard')); ?>" class="flex items-center gap-3 rounded-xl <?php echo e(request()->routeIs('cashier.dashboard') ? 'bg-indigo-500 text-white' : 'text-slate-600 hover:bg-slate-100'); ?> px-3 py-2">
                    <span class="material-symbols-outlined">point_of_sale</span>
                    <span class="font-semibold">Register</span>
                </a>
                <a href="<?php echo e(route('cashier.history')); ?>" class="flex w-full items-center gap-3 rounded-xl <?php echo e(request()->routeIs('cashier.history') ? 'bg-indigo-500 text-white' : 'text-slate-600 hover:bg-slate-100'); ?> px-3 py-2">
                    <span class="material-symbols-outlined">history</span>
                    <span class="font-semibold">History</span>
                </a>
                <a href="<?php echo e(route('cashier.history.supplier')); ?>" class="flex w-full items-center gap-3 rounded-xl <?php echo e(request()->routeIs('cashier.history.supplier') ? 'bg-indigo-500 text-white' : 'text-slate-600 hover:bg-slate-100'); ?> px-3 py-2">
                    <span class="material-symbols-outlined">account_tree</span>
                    <span class="font-semibold">PT/CV</span>
                </a>
                <a href="<?php echo e(route('cashier.drafts')); ?>" class="flex w-full items-center gap-3 rounded-xl <?php echo e(request()->routeIs('cashier.drafts') ? 'bg-indigo-500 text-white' : 'text-slate-600 hover:bg-slate-100'); ?> px-3 py-2">
                    <span class="material-symbols-outlined">draft</span>
                    <span class="font-semibold">Draft</span>
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($draftCount > 0): ?>
                        <span class="ml-auto rounded-full bg-slate-200 px-2 py-0.5 text-xs font-bold text-slate-700"><?php echo e($draftCount); ?></span>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3">
                    ...
                    <form method="POST" action="<?php echo e(route('cashier.cart.clear')); ?>">
                        <?php echo csrf_field(); ?>
                ...
                <div class="max-h-[calc(100vh-360px)] space-y-3 overflow-y-auto custom-scrollbar px-4 py-3">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $cartItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <div class="rounded-xl border border-slate-200 p-3">
                            <p class="text-sm font-semibold"><?php echo e($item['product_name']); ?></p>
                            <div class="mt-2 flex justify-between text-sm">
                                <form method="POST" action="<?php echo e(route('cashier.cart.update', $item['product_batch_id'])); ?>" class="flex flex-col gap-2" data-cart-item-form>
                                    <?php echo csrf_field(); ?>
                                    <div class="flex items-center gap-2">
                                        <label class="text-xs text-slate-500">Qty</label>
                                        <input type="number" min="0" name="qty" value="<?php echo e($item['qty']); ?>" class="w-16 rounded-lg border border-slate-300 px-2 py-1" data-cart-qty />
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <label class="text-xs text-slate-500">Harga</label>
                                        <input type="text" inputmode="numeric" name="price" value="<?php echo e(number_format((float) $item['price'], 0, ',', '.')); ?>" class="w-28 rounded-lg border border-slate-300 px-2 py-1" data-rupiah-input data-cart-price />
                                    </div>
                                    <button type="submit" class="w-fit rounded-lg border border-slate-300 px-2 py-1 text-xs">Update</button>
                                </form>
                                <div class="text-right">
                                    <span class="block font-bold" data-cart-line-total>Rp <?php echo e(number_format((float) $item['price'] * (int) $item['qty'], 0, ',', '.')); ?></span>
                                    <form method="POST" action="<?php echo e(route('cashier.cart.remove', $item['product_batch_id'])); ?>">
                                        <?php echo csrf_field(); ?>
            ...
            <div class="px-4 lg:px-6 py-3">
                <div class="flex items-center justify-between lg:hidden">
                    ...
                    <div class="flex items-center gap-2 text-slate-600">
                        <a href="<?php echo e(route('cashier.history')); ?>" class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-100">
                            <span class="material-symbols-outlined text-[18px]">history</span>
                        </a>
                        <a href="<?php echo e(route('cashier.drafts')); ?>" class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-100">
                            <span class="material-symbols-outlined text-[18px]">draft</span>
                        </a>
                        <a href="<?php echo e(route('cashier.history.supplier')); ?>" class="inline-flex h-8 w-8 items-center justify-center rounded-full bg-slate-100">
                            <span class="material-symbols-outlined text-[18px]">local_shipping</span>
                        </a>
                    </div>
                </div>
                <div class="mt-3 flex items-center gap-4">
                    <form id="search-form" method="GET" action="<?php echo e(route('cashier.dashboard')); ?>" class="relative flex-1">
                        <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">search</span>
                        <input id="product-search-input" type="text" name="q" value="<?php echo e($search); ?>" placeholder="Cari produk berdasarkan nama atau barcode..." autocomplete="off" class="h-11 w-full rounded-xl border border-slate-300 bg-slate-50 pl-10 pr-4 text-sm focus:border-emerald-500 focus:ring-emerald-500" />
                    ...
                    <div class="hidden lg:block text-right">
                        <p class="text-sm font-bold"><?php echo e($user?->name ?? 'Kasir'); ?></p>
                        ...
                        <article class="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                            <div class="relative h-36 md:h-44 bg-slate-100">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($image): ?>
                                    <img src="<?php echo e($image); ?>" alt="<?php echo e($product->name); ?>" class="h-full w-full object-cover" />
                                <?php else: ?>
                                    <div class="h-full w-full flex items-center justify-center text-slate-400">
                                        <span class="material-symbols-outlined text-5xl">inventory_2</span>
                                    </div>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                <span class="absolute right-2 top-2 rounded-lg px-2 py-1 text-xs font-semibold <?php echo e($stock <= 5 ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'); ?>">
                                    Stok: <?php echo e($stock); ?>

                                </span>
                            </div>
                            <div class="p-3">
                                <p class="text-[10px] font-bold uppercase tracking-tight text-emerald-700"><?php echo e($product->category?->name ?? '-'); ?></p>
                                <h3 class="font-bold text-slate-900 text-xl md:text-base line-clamp-1"><?php echo e($product->name); ?></h3>
                                <div class="mt-3 flex items-center justify-between">
                                    <p class="text-2xl md:text-2xl font-extrabold text-emerald-700">Rp <?php echo e(number_format($price, 0, ',', '.')); ?></p>
                                    <form method="POST" action="<?php echo e(route('cashier.cart.add', $product)); ?>">
                                        <?php echo csrf_field(); ?>
            ...
            <aside class="hidden xl:flex w-[390px] min-h-0 flex-col border-l border-slate-200 bg-white">
                <div class="min-h-0 flex-1 overflow-y-auto custom-scrollbar px-5 py-4 space-y-4">
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between"><span>Subtotal</span><span data-cart-subtotal>Rp <?php echo e(number_format($subtotal, 0, ',', '.')); ?></span></div>
                    </div>
                    <div class="mt-3 flex items-end justify-between">
                        <span class="text-xs tracking-wide text-slate-500">TOTAL HARGA</span>
                        <span class="text-4xl font-extrabold text-emerald-700" data-cart-total>Rp <?php echo e(number_format($cartTotal, 0, ',', '.')); ?></span>
                    </div>
                    <div class="mt-4 grid grid-cols-2 gap-3">
                        ...
                        <form method="POST" action="<?php echo e(route('cashier.cart.hold')); ?>">
                            <?php echo csrf_field(); ?>
                    ...
                    <form id="checkout-form-desktop" data-checkout-form method="POST" action="<?php echo e(route('cashier.checkout')); ?>" class="mt-3 space-y-2">
                        <?php echo csrf_field(); ?>
                        ...
                        <div class="grid grid-cols-[88px_minmax(0,1fr)] gap-2">
                            <select name="payment_method" data-payment-method class="w-full min-w-0 rounded-xl border border-slate-300 px-3 py-2 text-sm">
                                ...
                                <option value="transfer" <?php if(old('payment_method') === 'transfer'): echo 'selected'; endif; ?>>Transfer</option>
                                <option value="qris" <?php if(old('payment_method') === 'qris'): echo 'selected'; endif; ?>>QRIS</option>
                                <option value="debit" <?php if(old('payment_method') === 'debit'): echo 'selected'; endif; ?>>Debit</option>
                                <option value="credit" <?php if(old('payment_method') === 'credit'): echo 'selected'; endif; ?>>Credit</option>
                            </select>
                            <div class="min-w-0 space-y-1">
                                ...
                                <input
                                    ...
                                    inputmode="numeric"
                                    name="paid_amount"
                                    value="<?php echo e(old('payment_method') === 'credit' ? old('paid_amount', '0') : old('paid_amount', number_format((float) ceil($cartTotal), 0, ',', '.'))); ?>"
                        ...
                        <div data-credit-days-wrap class="<?php echo e(old('payment_method') === 'credit' ? '' : 'hidden'); ?> space-y-2">
                            <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input type="hidden" name="credit_due_date" value="<?php echo e(old('credit_due_date', '')); ?>" data-credit-due-input />
                                </div>
                            </div>
                            <p class="text-[11px] font-medium text-slate-500" data-credit-due-preview>Jatuh tempo otomatis akan dihitung dari hari ini.</p>
                        </div>
                        <input type="text" name="cashier_service_name" maxlength="100" value="<?php echo e(old('cashier_service_name', '')); ?>" autocomplete="off" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Nama pelayan / kasir" />
                        <input type="text" name="cashier_phone" maxlength="30" value="<?php echo e(old('cashier_phone', '')); ?>" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="No. telepon kasir" />
                        <input type="text" name="customer_name" maxlength="100" value="<?php echo e(old('customer_name', '')); ?>" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Nama pembeli (opsional)" />
                        <input type="text" name="customer_phone" maxlength="30" value="<?php echo e(old('customer_phone', '')); ?>" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="No. telepon pembeli (opsional)" />
...
<div class="xl:hidden fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white shadow-[0_-6px_16px_rgba(0,0,0,0.08)] safe-bottom">
    <div class="px-4 py-3 flex items-center justify-between bg-slate-50">
        <div class="leading-tight">
            ...
            <p class="font-bold text-slate-800" data-cart-subtotal>Rp <?php echo e(number_format($subtotal, 0, ',', '.')); ?></p>
        </div>
        <div class="flex gap-2">
            ...
            <a href="<?php echo e(route('cashier.drafts')); ?>" class="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-[11px] font-bold text-slate-600">DRAFT</a>
        </div>
    </div>
    <form id="checkout-form-mobile" data-checkout-form method="POST" action="<?php echo e(route('cashier.checkout')); ?>" class="px-4 pt-2 pb-4 space-y-2">
        <?php echo csrf_field(); ?>
        <input data-print-receipt-input type="hidden" name="print_receipt" value="0" />
        <input type="hidden" name="payment_method" value="<?php echo e(old('payment_method', 'cash')); ?>" />
        <input type="hidden" name="paid_amount" value="<?php echo e(old('payment_method') === 'credit' ? old('paid_amount', '0') : old('paid_amount', (int) ceil($cartTotal))); ?>" />
        <input type="hidden" name="credit_days" value="" />
        <input type="hidden" name="credit_due_date" value="<?php echo e(old('credit_due_date', '')); ?>" />
        <input type="hidden" name="cashier_service_name" value="" />
        <input type="hidden" name="cashier_phone" value="<?php echo e(old('cashier_phone', '')); ?>" />
        <input type="hidden" name="customer_name" value="<?php echo e(old('customer_name', '')); ?>" />
        <input type="hidden" name="customer_phone" value="<?php echo e(old('customer_phone', '')); ?>" />
        <div class="flex gap-2">
...
<div id="sale-confirm-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/60 px-4">
    <div class="w-full max-w-xl rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div id="mobile-confirm-fields" class="mt-3 hidden space-y-2 rounded-xl border border-slate-200 p-3">
            ...
            <div id="mobile-credit-days-wrap" class="<?php echo e(old('payment_method') === 'credit' ? '' : 'hidden'); ?> space-y-2">
                <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
                    <div class="space-y-1">
                        <label class="block text-[11px] font-semibold uppercase tracking-wide text-slate-500">Tempo Kredit</label>
            ...
            <input id="mobile-confirm-cashier-name" type="text" maxlength="100" value="<?php echo e(old('cashier_service_name', '')); ?>" autocomplete="off" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Nama pelayan / kasir" />
            <input id="mobile-confirm-cashier-phone" type="text" maxlength="30" value="<?php echo e(old('cashier_phone', '')); ?>" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="No. telepon kasir" />
            <input id="mobile-confirm-customer-name" type="text" maxlength="100" value="<?php echo e(old('customer_name', '')); ?>" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Nama pembeli (opsional)" />
            <input id="mobile-confirm-customer-phone" type="text" maxlength="30" value="<?php echo e(old('customer_phone', '')); ?>" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="No. telepon pembeli (opsional)" />
        </div>
...
<script>
    ...
    const subtotal = Number(<?php echo json_encode($subtotal, 15, 512) ?>);
    const total = Number(<?php echo json_encode($cartTotal, 15, 512) ?>);
    const searchSuggestions = <?php echo json_encode($searchSuggestions, 15, 512) ?>;
    const nextResetAtIso = <?php echo json_encode($nextResetAtIso, 15, 512) ?>;
...
<?php /**PATH C:\laragon\www\backend\resources\views/cashier/dashboard.blade.php ENDPATH**/ ?>