...
<head>
    ...
    <title>Form Retur - <?php echo e($sale->invoice_number); ?></title>
...
<main class="mx-auto max-w-5xl p-4 lg:p-6">
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <p class="text-sm text-slate-500">Invoice: <?php echo e($sale->invoice_number); ?> | Kasir: <?php echo e($user?->name); ?></p>
        </div>
    ...
    <div class="mb-4 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-4">
        <div>
            ...
            <p class="font-semibold"><?php echo e($sale->created_at?->format('d M Y H:i')); ?></p>
        </div>
        <div>
            <p class="text-xs uppercase text-slate-500">Pembeli</p>
            <p class="font-semibold"><?php echo e($sale->customer_name ?: '-'); ?></p>
        </div>
        <div>
            <p class="text-xs uppercase text-slate-500">Metode</p>
            <p class="font-semibold uppercase"><?php echo e($sale->payment_method); ?></p>
        </div>
        <div>
    ...
    <form method="POST" action="<?php echo e(route('cashier.return.store', $sale)); ?>" class="rounded-2xl border border-slate-200 bg-white">
        <?php echo csrf_field(); ?>
            ...
            <table class="min-w-[980px] w-full text-xs md:text-sm">
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $sale->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                    ...
                    <tr
                        ...
                        data-product-name="<?php echo e(e($item->product_name)); ?>"
                        data-price="<?php echo e((float) $item->price); ?>"
                        data-sold-qty="<?php echo e((int) $item->qty); ?>"
                        data-returned-qty="<?php echo e($returnedQty); ?>"
                        data-max-return="<?php echo e($maxReturn); ?>"
                    >
                        <td class="px-3 py-3 align-top">
                            <label class="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-2.5 py-2">
                                <input
                                    ...
                                    data-index="<?php echo e($index); ?>"
                                    <?php if(old("items.{$index}.qty", 0) > 0): echo 'checked'; endif; ?>
                                    <?php if($maxReturn === 0): echo 'disabled'; endif; ?>
                                />
                            ...
                            <div class="js-return-qty-wrap mt-2 hidden w-fit rounded-xl border border-slate-200 bg-white px-2 py-1">
                                <div class="flex items-center gap-2">
                                    <button type="button" class="js-return-qty-minus rounded-lg border border-slate-300 px-2 py-1 text-sm font-bold text-slate-700" data-index="<?php echo e($index); ?>">-</button>
                                    <input
                                        type="text"
                                        class="js-return-qty-display w-12 border-0 bg-transparent text-center text-sm font-semibold text-slate-900 focus:ring-0"
                                        data-index="<?php echo e($index); ?>"
                                        value="<?php echo e(old("items.{$index}.qty", 0)); ?>"
                                        readonly
                                    />
                                    <button type="button" class="js-return-qty-plus rounded-lg border border-slate-300 px-2 py-1 text-sm font-bold text-slate-700" data-index="<?php echo e($index); ?>">+</button>
                                </div>
                            </div>
                            <input type="hidden" name="items[<?php echo e($index); ?>][sale_item_id]" value="<?php echo e($item->id); ?>" />
                            <input type="hidden" name="items[<?php echo e($index); ?>][qty]" value="<?php echo e(old("items.{$index}.qty", 0)); ?>" class="js-return-qty" data-index="<?php echo e($index); ?>" />
                            <input type="hidden" name="items[<?php echo e($index); ?>][replacement_product_id]" value="<?php echo e(old("items.{$index}.replacement_product_id", '')); ?>" class="js-replacement-product-id" data-index="<?php echo e($index); ?>" />
                            <input type="hidden" name="items[<?php echo e($index); ?>][replacement_batch_id]" value="<?php echo e(old("items.{$index}.replacement_batch_id", '')); ?>" class="js-replacement-batch-id" data-index="<?php echo e($index); ?>" />
                            <input type="hidden" name="items[<?php echo e($index); ?>][replacement_qty]" value="<?php echo e(old("items.{$index}.replacement_qty", '')); ?>" class="js-replacement-qty" data-index="<?php echo e($index); ?>" />
                            <input type="hidden" name="items[<?php echo e($index); ?>][replacement_label]" value="<?php echo e(old("items.{$index}.replacement_label", '')); ?>" class="js-replacement-label" data-index="<?php echo e($index); ?>" />
                            <input type="hidden" name="items[<?php echo e($index); ?>][replacement_price]" value="<?php echo e(old("items.{$index}.replacement_price", '')); ?>" class="js-replacement-price" data-index="<?php echo e($index); ?>" />
                        </td>
                        <td class="px-3 py-3">
                            <p class="font-semibold"><?php echo e($item->product_name); ?></p>
                        </td>
                        <td class="px-3 py-3 text-right"><?php echo e($item->qty); ?></td>
                        <td class="px-3 py-3 text-right"><?php echo e($returnedQty); ?></td>
                        <td class="px-3 py-3 text-right font-semibold"><?php echo e($maxReturn); ?></td>
                        <td class="px-3 py-3 text-right">Rp <?php echo e(number_format((float) $item->price, 0, ',', '.')); ?></td>
                    </tr>
                    <tr class="js-replacement-panel hidden border-b border-slate-100 bg-[#fffaf0]" data-index="<?php echo e($index); ?>" data-old-lines='<?php echo json_encode(old("items.{$index}.replacement_lines", []), 512) ?>'>
                        <td colspan="6" class="px-3 py-3">
                            <div class="rounded-xl border border-amber-200 bg-white p-3">
                                ...
                                <div class="js-replacement-lines mt-3 space-y-3" data-index="<?php echo e($index); ?>"></div>
                            </div>
                        </td>
                    </tr>
                ...
                <div class="mt-3 grid gap-4 md:grid-cols-[1fr_0.9fr]">
                    <div>
                        ...
                        <input
                            ...
                            type="text"
                            inputmode="numeric"
                            value="<?php echo e(old('extra_payment_amount', '')); ?>"
            ...
            <div class="grid gap-4 md:grid-cols-[1.2fr_0.8fr]">
                <div>
                    ...
                    <select id="return_reason" name="return_reason" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
                        <option value="barang_rusak" <?php if($selectedReason === 'barang_rusak'): echo 'selected'; endif; ?>>Barang rusak</option>
                        <option value="ganti_barang" <?php if($selectedReason === 'ganti_barang'): echo 'selected'; endif; ?>>Ganti barang</option>
                        <option value="pengembalian_dana" <?php if($selectedReason === 'pengembalian_dana'): echo 'selected'; endif; ?>>Pengembalian dana</option>
                        <option value="salah_kirim" <?php if($selectedReason === 'salah_kirim'): echo 'selected'; endif; ?>>Salah kirim</option>
                        <option value="lainnya" <?php if($selectedReason === 'lainnya'): echo 'selected'; endif; ?>>Lainnya</option>
                    </select>
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3" maxlength="1000" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Isi detail tambahan bila diperlukan"><?php echo e(old('notes')); ?></textarea>
                </div>
            </div>
...
<script>
    const replacementOptions = <?php echo json_encode($replacementOptions, 15, 512) ?>;
    ...
    const paymentMethod = '<?php echo e(strtolower((string) $sale->payment_method)); ?>';
...
<?php /**PATH C:\laragon\www\backend\resources\views\cashier\return-form.blade.php ENDPATH**/ ?>