...
<div class="h-screen overflow-hidden bg-[#f7f9fb]">
    <aside class="hidden lg:flex fixed inset-y-0 left-0 z-30 w-[340px] flex-col border-r border-slate-300 bg-white">
        ...
        <div class="min-h-0 flex-1 overflow-y-auto custom-scrollbar px-4 py-4 space-y-4">
            <nav class="space-y-2">
                <a href="<?php echo e(route('cashier.dashboard')); ?>" class="flex items-center gap-3 rounded-xl <?php echo e($isTransactionDashboard ? 'bg-indigo-500 text-white' : 'text-slate-600 hover:bg-slate-100'); ?> px-3 py-2">
                ...
                <div class="max-h-[calc(100vh-360px)] space-y-3 overflow-y-auto custom-scrollbar px-4 py-3">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $cartItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <div class="rounded-xl border border-slate-200 p-3">
                            ...
                            <p class="mt-1 text-xs text-slate-500">Part No: <?php echo e($item['part_number']); ?></p>
                            <p class="mt-1 text-xs text-slate-500">
                                Stok batch: <?php echo e(number_format((int) ($item['batch_stock'] ?? 0), 0, ',', '.')); ?>

                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if((int) ($item['available_stock'] ?? 0) > (int) ($item['batch_stock'] ?? 0)): ?>
                                    | total gabungan: <?php echo e(number_format((int) ($item['available_stock'] ?? 0), 0, ',', '.')); ?>

                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </p>
                            <div class="mt-2 flex justify-between text-sm">
                                <form method="POST" action="<?php echo e(route('cashier.cart.update', $item['product_batch_id'])); ?>" class="flex flex-col gap-2" data-cart-item-form data-merge-stock="<?php echo e(!empty($item['merge_stock']) ? '1' : '0'); ?>">
                                    <?php echo csrf_field(); ?>
                                    <div class="flex items-center gap-2">
                                        <label class="text-xs text-slate-500">Qty</label>
                                        <input type="number" min="0" max="<?php echo e((int) ($item['max_qty'] ?? 0)); ?>" name="qty" value="<?php echo e($item['qty']); ?>" class="w-16 rounded-lg border border-slate-300 px-2 py-1" data-cart-qty data-max-stock="<?php echo e((int) ($item['max_qty'] ?? 0)); ?>" />
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <label class="text-xs text-slate-500"><?php echo e(!empty($item['merge_stock']) ? 'Total Harga' : 'Harga'); ?></label>
                                        <input
                                            type="text"
                                            inputmode="numeric"
                                            name="price"
                                            value="<?php echo e(number_format((float) ($item['merge_stock'] ? ($item['line_total'] ?? ((float) $item['price'] * (int) $item['qty'])) : $item['price']), 0, ',', '.')); ?>"
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if((int) ($item['can_merge_stock'] ?? 0) === 1): ?>
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(empty($item['merge_stock'])): ?>
                                                <button
                                                    type="submit"
                                                    formaction="<?php echo e(route('cashier.cart.merge', $item['product_batch_id'])); ?>"
                                ...
                                <div class="text-right">
                                    <span class="block font-bold" data-cart-line-total>Rp <?php echo e(number_format((float) ($item['line_total'] ?? ((float) $item['price'] * (int) $item['qty'])), 0, ',', '.')); ?></span>
            ...
            <div class="px-4 lg:px-6 py-3">
                ...
                <div class="mt-3 flex items-center gap-4">
                    ...
                    <div class="hidden lg:flex items-center gap-3">
                        <div class="text-right">
                            <p class="text-sm font-bold"><?php echo e($user?->name ?? 'Admin'); ?></p>
                        ...
                        <article class="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
                            ...
                            <div class="p-3">
                                ...
                                <div class="mt-3 flex items-center justify-between">
                                    ...
                                    <form method="POST" action="<?php echo e(route('cashier.cart.add', $batch)); ?>">
                                        <?php echo csrf_field(); ?>
            ...
            <aside class="hidden xl:flex w-[390px] min-h-0 flex-col border-l border-slate-200 bg-white">
                <div class="min-h-0 flex-1 overflow-y-auto custom-scrollbar px-5 py-4 space-y-4">
                    ...
                    <form id="checkout-form-desktop" data-checkout-form method="POST" action="<?php echo e(route('cashier.checkout')); ?>" class="mt-3 space-y-2">
                        ...
                        <input type="text" name="cashier_service_name" maxlength="100" value="<?php echo e(old('cashier_service_name', '')); ?>" autocomplete="off" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Nama petugas / admin" />
...
<div id="sale-confirm-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/60 px-4">
    <div class="w-full max-w-xl rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div id="mobile-confirm-fields" class="mt-3 hidden space-y-2 rounded-xl border border-slate-200 p-3">
            ...
            <input id="mobile-confirm-cashier-name" type="text" maxlength="100" value="<?php echo e(old('cashier_service_name', '')); ?>" autocomplete="off" class="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm" placeholder="Nama petugas / admin" />
...
<?php /**PATH /home/mrgana/pos-inventory/backend/resources/views/cashier/dashboard.blade.php ENDPATH**/ ?>